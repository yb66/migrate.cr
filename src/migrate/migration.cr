...
module Migrate
  ...
  class Migration
    abstract struct Statement
      ...
      # Check if text would result in an empty statement after processing
      def self.would_be_empty?(text : String) : Bool
        ...
        if captured = s.check_until(/;/)
          ...
          when COMPLEX_STOPPER
            ...
            if complex_statement = s.scan_until /#{COMPLEX_END}|\z/
              ...
              unless Statement.would_be_empty?(complex_statement)
                @statements << state.new(complex_statement)
              end
            end
          else # just use it
            chunk = s.scan_until(/;/).not_nil!
            unless Statement.would_be_empty?(chunk)
              @statements << state.new(chunk)
            end
          end
        end
      end
